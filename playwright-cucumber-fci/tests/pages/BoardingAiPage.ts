...
type FinalAction = 'SAVE_LSA' | 'SEND_TO_BOARDING';
...
export class BoardingAiPage {
  ...
  private async clickFinalActionButton(timeoutMs = 30000): Promise<FinalAction> {
    const start = Date.now();

    // candidatos (regex tolerante)
    ...
    while (Date.now() - start < timeoutMs) {
      // 1) Save data & next to generate LSA
  ...
  /**
   * Verifica si aparece el toast-error "Invalid value of Borrower Delivery Options."
   * Ideal para llamarlo inmediatamente después de clickear "Send to Boarding".
   *
   * - Si aparece: log + screenshot + throw Error (falla el test con motivo claro)
   * - Si no aparece: no hace nada
   */
  async assertNoBorrowerDeliveryOptionsToast(timeoutMs = 8000) {
    const pattern = /Invalid value of Borrower Delivery Options\./i;

    // Locator tolerante (con o sin #toast-container)
    const toastMsg = this.page
      .locator('#toast-container .toast.toast-error .toast-message, .toast.toast-error .toast-message')
      .filter({ hasText: pattern })
      .first();

    // Esperar "un rato corto" a que aparezca
    const appeared = await toastMsg
    ...
    const msg = (await toastMsg.innerText().catch(() => '')).trim();
    console.warn(`❌ Toast error detectado luego de Send to Boarding: ${msg}`);

    const shot = `debug-toast-borrower-delivery-${Date.now()}.png`;
    await this.page.screenshot({ path: shot, fullPage: true });

    throw new Error(
      `Se mostró un toast de error luego de "Send to Boarding": "${msg}". Screenshot: ${shot}`
    );
  }
  ...
  async signAndSave() {
    ...
    // ✅ Click final y detecto cuál fue
    const action = await this.clickFinalActionButton(30000);

    // ✅ Solo si fue Send to Boarding, chequeo toast y corto flujo si aparece
    if (action === 'SEND_TO_BOARDING') {
      await this.assertNoBorrowerDeliveryOptionsToast(30000);
    }
